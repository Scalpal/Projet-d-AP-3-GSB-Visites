...
if (connexionDAO::isLoggedOn()){
    ...
    $Visiteur= VisiteurDAO::get_visiteurByLogin($_SESSION['login']); // Obligatoire pour que les infos du visiteur sur la navbar soient affichées
    $idVisiteur = $Visiteur[0]->get_IdVisiteur(); // récupération de l'ID du visiteur 
    $nomVisiteur = $Visiteur[0]->get_NomVisiteur(); // récupération du nom du visiteur pour affichage message d'accueil
    $prenomVisiteur = $Visiteur[0]->get_PrenomVisiteur(); // récupération du prénom du visiteur pour affichage message d'accueil
    ...
    require "$racine/vue/vueWelcomeMessage.php"; 

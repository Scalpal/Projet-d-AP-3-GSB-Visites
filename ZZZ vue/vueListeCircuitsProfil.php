...
if($lesCircuits != null){ // Vérifie si il y'a au moins 1 circuit à afficher (pour éviter les erreurs)
  ...
  for ($i=0 ; $i<count($lesCircuits); $i++){
    ...
    ?><p id="InfoReserv"><?php echo("Réservation n°".$IdReservation[$i]->get_IdRes()); ?></p>
    ...
    <div id="infosReservation">
      ...
      <form action="./?action=annulation" method="post">
        <input type="submit" value="Annuler" id="circuitReserve" /> 
      </form>
  ...
  <?php echo "Vous n'avez pas encore réservé de circuits, c'est le moment d'y songer !"; ?>
  </p>
<?php } ?>
